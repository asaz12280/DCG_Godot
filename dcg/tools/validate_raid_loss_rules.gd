...
func _initialize() -> void:
	...
	if _errors.is_empty():
		print("[raid_loss_rules] OK lost=backpack kept=safe_pocket stash=unchanged boundaries=clean")
		quit(0)
	else:
...
func _validate_loss_context_from_player() -> void:
	...
	if _stack_quantity(kept_items, AmmoItem.resource_path) != 8:
		_errors.append("Death loss rules should keep safe pocket items.")
	if str(context.get("loss_rule", "")) != "backpack_lost_safe_pocket_kept":
...
func _validate_death_applier_clears_raid_inventory_only() -> void:
	...
	if _stack_quantity(stash, AmmoItem.resource_path) != 2:
		_errors.append("Death should preserve existing base stash items.")
...
func _validate_source_boundaries() -> void:
	...
	for required in ["collect_backpack_items", "collect_safe_pocket_items", "lost_items", "kept_safe_pocket_items"]:
